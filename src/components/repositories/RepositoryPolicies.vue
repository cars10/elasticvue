<template>
    ...
    <q-dialog v-model="dialog" transition-duration="100">
        <q-card style="width: 800px; max-width: 90vw;">
            ...
            <q-table v-if="policies.length > 0" flat dense row-key="id" :columns="columns" :rows="policies"
                :pagination="{ sortBy: 'name' }" :rows-per-page-options="DEFAULT_ROWS_PER_PAGE">
                <template #body="{ row }">
                    <tr>
                        <td>{{ row.id }}</td>
                        <td>{{ row.name }}</td>
...
const retentionFormat = (retention: SnapshotPolicyRetention | undefined) => {
    if (!retention) return ''
    return [retention?.expire_after, retention?.max_count ? `max ${retention?.max_count}` : '',
    retention?.min_count ? `min ${retention?.min_count}` : ''].filter(Boolean).join(', ')
}
