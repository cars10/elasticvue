              ...
              <div class="d-inline-block float-right">
                ...
                >
                  <v-icon
                    v-if="onlyFavorites"
                    :title="
                      $t('query.rest_query_history.favorites_filter.show_all')
                    "
                  >
                    mdi-star
                  </v-icon>
                  <v-icon
                    ...
                    :title="
                      $t(
                        'query.rest_query_history.favorites_filter.only_favorites'
                      )
                    "
                  >
                    mdi-star-outline
                ...
                <td>
                  <v-btn icon @click.stop="favoriteItem(item)">
                    <v-icon
                      ...
                      :title="
                        $t('query.rest_query_history.table.row.favorite.remove')
                      "
                    >
                      mdi-star
                    </v-icon>
                    <v-icon
                      v-else
                      :title="
                        $t('query.rest_query_history.table.row.favorite.add')
                      "
                    >
                      mdi-star-outline
                    </v-icon>
                  ...
                  <template v-else
                    ><span class="font--mono">{{ item.method }}</span>
                    {{ item.path }}</template
                  >
                </td>
              ...
              <v-menu v-if="items.length > 0" offset-y>
                ...
                <v-list>
                  <v-list-item @click="clearNonFavorites">
                    <v-list-item-title>{{
                      $t("query.rest_query_history.clear.only_non_favorites")
                    }}</v-list-item-title>
                  </v-list-item>
                  <v-list-item @click="clearAll">
                    <v-list-item-title>{{
                      $t("query.rest_query_history.clear.all")
                    }}</v-list-item-title>
                  </v-list-item>
                </v-list>
              </v-menu>
          ...
          <template v-if="selectedItem">
            ...
            <div>
              <v-btn
                ...
                @click="apply(selectedItem)"
              >
                {{ $t("query.rest_query_history.body_preview.use") }}
              </v-btn>
              <v-btn
                :title="
                  $t('query.rest_query_history.body_preview.remove_entry')
                "
                class="mt-4 ml-2"
                icon
              ...
              <button
                class="btn-link float-right mt-4"
                @click="selectItem(null)"
              >
                <small>{{
                  $t("query.rest_query_history.body_preview.unselect")
                }}</small>
              </button>
            </div>
          </template>
