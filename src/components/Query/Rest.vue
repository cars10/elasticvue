  ...
  <div>
    <div class="mb-12">
      <v-btn class="pl-1 mr-2" @click="historyCollapsed = !historyCollapsed">
        <v-icon>{{
          historyCollapsed ? "mdi-chevron-up" : "mdi-chevron-down"
        }}</v-icon>
    ...
    <v-form @submit.prevent="loadData">
      ...
      <v-row>
        <v-col :md="vertical ? 12 : 6" cols="12">
          ...
          <div v-else>
            <div
              :style="vertical ? 'height: 200px' : 'height: 500px'"
              class="mb-4"
            >
              <v-alert :value="true" class="request-body-disabled-hint">
                <p>
                  <span
                    v-html="
                      $t('query.rest.get_request_hint.cannot_send_body', {
