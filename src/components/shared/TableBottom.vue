<template>
  <div class="flex col-grow justify-end">
    ...
    <div class="q-table__control">
      ...
      <div v-else>{{ firstRowIndex }}-{{ total }} of {{ scope.pagination.rowsNumber || total }}</div>
    </div>
    ...
    <div class="q-table__control">
      ...
      <q-btn icon="chevron_left" color="grey-8" size="sm" round dense flat :disable="scope.isFirstPage" @click="scope.prevPage" />

      <q-btn icon="chevron_right" color="grey-8" size="sm" round dense flat :disable="scope.isLastPage" @click="scope.nextPage" />
...
const lastRowIndex = computed(() =>
  Math.min(props.scope.pagination.page * props.scope.pagination.rowsPerPage, props.scope.pagination.rowsNumber || props.total)
)
</script>
